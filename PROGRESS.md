...
| 7 | Storefront (Next.js) | 🟡 in progress | — (listing + full configurator with live itemized pricing, share links, SEO fundamentals live; still needed: media previews, size guide UI, i18n/currency switch, cart + Mollie deposit checkout) |
...
| 2026-07-02 | **Phase 7 (first slice).** `storefront/` (Next.js 15, port 3000): brand/market/currency read from tenant settings (never hardcoded — decision #1 open), `/rings` listing from /api/v1, `/rings/[code]` configurator — data-driven steps from option groups in step order, chip selection, live itemized price on every change, invalid selections show rule messages, quote_only shows estimated range + request-a-quote, non-returnable warning surfaces, share button (engine share-state → `?c=` URL reproduces exactly). SEO fundamentals server-rendered: per-page title/meta/canonical + ProductGroup JSON-LD from engine data. Engine: GET /api/v1/templates/by-code/{code}. Verified live end-to-end. |
| 2026-07-02 | **Phase 6 complete.** `app/api/public.py` (/api/v1): catalog reads (categories, templates + components + option groups with defaults, published rules for client hinting, ring sizes); POST /config/validate and /config/price per the spec-13 response contract (validity/itemized/total/estimated_range/weight/config_hash, never NaN); POST /config/share + GET /config/{code} via urlsafe-base64 state (no storage, exact reproduction, noindex). `app/services/config_service.py`: config_hash embeds the latest metal-snapshot stamp (a refresh self-invalidates every cached price); Redis response cache is fail-open (Redis down = slower, never wrong). Redis added to CI. Exit verified: FRD criteria 1–3 via API, identical-input determinism, p95 < 300 ms. |
